...
describe("server-side RBAC policy", () => {
  ...
  it("prevents team leaders from reviewing teams outside their resolved leadership scope", () => {
    ...
    expect(canReviewTeam("DEPARTMENT_HEAD", "team-payment", [])).toBe(true);
  });

  it("limits member evaluation reads to self while honoring resolved team leadership", () => {
    expect(canReadMemberEvaluation({ role: "MEMBER", actorMemberId: "member-1", targetMemberId: "member-1", targetTeamId: "team-api", ledTeamIds: [] })).toBe(true);
    expect(canReadMemberEvaluation({ role: "MEMBER", actorMemberId: "member-1", targetMemberId: "member-2", targetTeamId: "team-api", ledTeamIds: [] })).toBe(false);
    expect(canReadMemberEvaluation({ role: "TEAM_LEADER", actorMemberId: "leader-1", targetMemberId: "member-2", targetTeamId: "team-api", ledTeamIds: ["team-api"] })).toBe(true);
    expect(canReadMemberEvaluation({ role: "TEAM_LEADER", actorMemberId: "leader-1", targetMemberId: "member-2", targetTeamId: "team-payment", ledTeamIds: ["team-api"] })).toBe(false);
  });
});
