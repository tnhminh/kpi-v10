...
import { finalizeEvaluation } from "@/server/review/repository";
...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const { user, access } = await requireOrganizationPermission(request, organizationId, "evaluation:finalize");
    const requestId = getRequestId(request.headers);
    const result = await finalizeEvaluation({ organizationId, evaluationId, actorUserId: user.id, actorRole: access.role, requestId });
