...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const result = await completeReview({ organizationId, evaluationId, actorUserId: user.id, actorRole: access.role, requestId, layer: "DEPARTMENT_HEAD", adjustments: payload.adjustments });
