...
import { evaluatePeriodMembers } from "@/server/evaluation/repository";
import { evaluatePeriodSchema } from "@/server/evaluation/validation";
...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const { user } = await requireOrganizationPermission(request, organizationId, "evaluation:ingest");
    const payload = await parseJson(request, evaluatePeriodSchema);
    return ok(request, await evaluatePeriodMembers({ organizationId, periodId, actorUserId: user.id, members: payload.members }));
  } catch (error) {
