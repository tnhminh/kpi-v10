...
import { createMember, listMembers } from "@/server/organization/repository";
import { createMemberSchema, parseUuidParam } from "@/server/organization/validation";
...
export async function GET(request: Request, { params }: Context) {
  try {
    ...
    return ok(request, await listMembers(organizationId));
  } catch (error) {
...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const payload = await parseJson(request, createMemberSchema);
    const requestId = getRequestId(request.headers);
    const created = await createMember({ organizationId, actorUserId: user.id, requestId, ...payload });
