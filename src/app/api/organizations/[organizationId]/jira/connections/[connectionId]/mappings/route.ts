...
import { listJiraMappings, replaceJiraMappings } from "@/server/jira/repository";
import { replaceJiraMappingsSchema } from "@/server/jira/validation";
...
export async function GET(request: Request, { params }: Context) {
  try {
    ...
    return ok(request, await listJiraMappings(organizationId, connectionId));
  } catch (error) {
...
export async function PUT(request: Request, { params }: Context) {
  try {
    ...
    const payload = await parseJson(request, replaceJiraMappingsSchema);
    const requestId = getRequestId(request.headers);
    const result = await replaceJiraMappings({ organizationId, connectionId, actorUserId: user.id, requestId, mappings: payload.mappings });
