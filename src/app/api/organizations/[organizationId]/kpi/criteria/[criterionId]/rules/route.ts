...
import { replaceCriterionScoringRules } from "@/server/kpi/repository";
import { replaceScoringRulesSchema } from "@/server/kpi/validation";
...
export async function PUT(request: Request, { params }: Context) {
  try {
    ...
    const payload = await parseJson(request, replaceScoringRulesSchema);
    const requestId = getRequestId(request.headers);
    return ok(request, await replaceCriterionScoringRules({ organizationId, criterionId, actorUserId: user.id, requestId, rules: payload.rules }), 200, requestId);
  } catch (error) {
