...
import { setCriterionMetricConfiguration } from "@/server/kpi/repository";
import { metricConfigurationSchema } from "@/server/kpi/validation";
...
export async function PUT(request: Request, { params }: Context) {
  try {
    ...
    const payload = await parseJson(request, metricConfigurationSchema);
    const requestId = getRequestId(request.headers);
    return ok(request, await setCriterionMetricConfiguration({ organizationId, criterionId, actorUserId: user.id, requestId, ...payload }), 200, requestId);
  } catch (error) {
