...
import { listOrganizationUsers, provisionOrganizationUser } from "@/server/organization/admin-repository";
import { requireOrganizationPermission } from "@/server/organization/access";
import { parseUuidParam, provisionOrganizationUserSchema } from "@/server/organization/validation";
...
export async function GET(request: Request, { params }: Context) {
  try {
    ...
    return ok(request, await listOrganizationUsers(organizationId));
  } catch (error) {
...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const payload = await parseJson(request, provisionOrganizationUserSchema);
    const requestId = getRequestId(request.headers);
    return ok(request, await provisionOrganizationUser({ organizationId, actorUserId: user.id, requestId, ...payload }), 201, requestId);
  } catch (error) {
