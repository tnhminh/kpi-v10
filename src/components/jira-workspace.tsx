  ...
  return <div>
    ...
    <div className="grid gap-4 xl:grid-cols-[310px_1fr]">
      <div className="space-y-4">
        ...
        <Panel className="p-4"><div className="mb-3 flex items-center gap-2"><Plus size={15}/><h2 className="text-sm font-semibold">Add Jira Cloud</h2></div><label className="text-[10px] font-bold uppercase text-slate-400">Workspace URL</label><input aria-label="Workspace URL" value={workspaceUrl} onChange={(event) => setWorkspaceUrl(event.target.value)} placeholder="https://company.atlassian.net" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs outline-none"/><label className="mt-3 block text-[10px] font-bold uppercase text-slate-400">Credential reference</label><div className="relative mt-1"><KeyRound size={13} className="absolute left-2.5 top-2.5 text-slate-400"/><input aria-label="Credential reference" value={secretRef} onChange={(event) => setSecretRef(event.target.value)} placeholder="env:JIRA_BACKEND_CREDENTIALS" className="w-full rounded-lg border border-slate-200 py-2 pl-8 pr-3 text-xs outline-none"/></div><p className="mt-1 text-[9px] leading-4 text-slate-400">Store JSON credentials in the runtime secret source. The database/API stores only this reference.</p><label className="mt-3 block text-[10px] font-bold uppercase text-slate-400">JQL</label><textarea aria-label="JQL" value={jql} onChange={(event) => setJql(event.target.value)} rows={2} maxLength={4000} className="mt-1 w-full resize-none rounded-lg border border-slate-200 px-3 py-2 text-xs outline-none"/><label className="mt-3 block text-[10px] font-bold uppercase text-slate-400">Extra fields</label><input aria-label="Extra fields" value={fields} onChange={(event) => setFields(event.target.value)} placeholder="duedate, resolutiondate, customfield_10016" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs outline-none"/><label className="mt-3 block text-[10px] font-bold uppercase text-slate-400">Fact mappings JSON</label><textarea aria-label="Fact mappings JSON" value={factMappingsJson} onChange={(event) => setFactMappingsJson(event.target.value)} rows={4} spellCheck={false} className="mt-1 w-full resize-y rounded-lg border border-slate-200 px-3 py-2 font-mono text-[10px] outline-none"/><button disabled={createMutation.isPending || !workspaceUrl.trim() || !secretRef.trim() || !jql.trim()} onClick={() => createMutation.mutate()} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50">{createMutation.isPending && <LoaderCircle size={13} className="animate-spin"/>} Save connection</button></Panel>
      </div>
      ...
      {!selected ? <Panel className="grid min-h-[520px] place-items-center p-8 text-center"><div><Database size={28} className="mx-auto text-slate-300"/><div className="mt-3 font-semibold">Configure a Jira workspace</div><p className="mt-1 text-xs text-slate-500">Then map Jira accounts to members and run a traceable sync.</p></div></Panel> : <div>
        ...
        <Panel className="mt-4 overflow-hidden"><div className="overflow-x-auto border-b border-slate-200 bg-slate-50"><div className="flex min-w-max px-3">{(["overview", "mapping", "facts", "runs"] as const).map((item) => <button key={item} onClick={() => setTab(item)} className={`border-b-2 px-4 py-3 text-xs font-semibold capitalize ${tab === item ? "border-blue-600 text-blue-700" : "border-transparent text-slate-500"}`}>{item}</button>)}</div></div>
          {tab === "overview" && <div className="p-5"><div className="grid gap-4 md:grid-cols-2"><div className="rounded-xl border border-slate-200 p-4"><div className="text-[10px] font-bold uppercase text-slate-400">Workspace</div><div className="mt-2 text-sm font-semibold">{selected.workspaceUrl}</div><div className="mt-3 flex gap-2"><span className="rounded-md bg-emerald-50 px-2 py-1 text-[10px] font-semibold text-emerald-700">{selected.active ? "ACTIVE" : "INACTIVE"}</span><span className="rounded-md bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-600">{selected.secretConfigured ? "SECRET REFERENCE CONFIGURED" : "NO SECRET REFERENCE"}</span></div></div><div className="rounded-xl border border-slate-200 p-4"><div className="text-[10px] font-bold uppercase text-slate-400">Sync policy</div><div className="mt-2 font-mono text-[11px] text-slate-700">{selected.syncConfig.jql}</div><div className="mt-3 text-[10px] text-slate-500">{Object.keys(selected.syncConfig.factMappings).length} declarative fact mappings · {selected.syncConfig.fields.length} extra fields</div></div></div><div className="mt-4 rounded-xl border border-blue-100 bg-blue-50/60 p-4"><div className="flex gap-3"><ShieldCheck size={18} className="mt-0.5 text-blue-700"/><div><div className="text-sm font-semibold text-blue-950">Historical safety boundary</div><p className="mt-1 text-xs leading-5 text-blue-800">This page manages mutable current Jira facts. The evaluation pipeline snapshots the exact facts used by each evaluation; those snapshots are database-immutable, so later Jira changes cannot silently rewrite reviewed results.</p></div></div></div></div>}
          {tab === "mapping" && <div className="p-5"><div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-sm font-semibold">Member ↔ Jira account mapping</h3><p className="text-[10px] text-slate-500">Explicit identity attribution only. Display-name guessing is not authority.</p></div><div className="flex gap-2"><button onClick={() => setMappingDrafts((rows) => [...rows, { memberId: "", jiraAccountId: "", jiraDisplayName: "" }])} className="flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold"><Plus size={12}/> Row</button><button disabled={saveMappingsMutation.isPending} onClick={() => saveMappingsMutation.mutate()} className="flex items-center gap-1 rounded-lg bg-slate-950 px-2.5 py-1.5 text-xs font-semibold text-white"><Save size={12}/> Save</button></div></div><div className="space-y-2">{mappingDrafts.length === 0 && <div className="rounded-lg bg-slate-50 p-5 text-center text-xs text-slate-500">No account mappings yet.</div>}{mappingDrafts.map((row, index) => <div key={`${index}-${row.memberId}`} className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_1fr_1fr_32px]"><select aria-label={`Jira mapping member ${index + 1}`} value={row.memberId} onChange={(event) => setMappingDrafts((rows) => rows.map((current, i) => i === index ? { ...current, memberId: event.target.value } : current))} className="rounded-lg border border-slate-200 px-2 py-2 text-xs"><option value="">Select member…</option>{(membersQuery.data ?? []).map((member) => <option key={member.id} value={member.id}>{member.name} · {member.employeeId}</option>)}</select><input aria-label={`Jira account ID ${index + 1}`} value={row.jiraAccountId} onChange={(event) => setMappingDrafts((rows) => rows.map((current, i) => i === index ? { ...current, jiraAccountId: event.target.value } : current))} placeholder="Jira account ID" className="rounded-lg border border-slate-200 px-2 py-2 text-xs"/><input aria-label={`Jira display name ${index + 1}`} value={row.jiraDisplayName} onChange={(event) => setMappingDrafts((rows) => rows.map((current, i) => i === index ? { ...current, jiraDisplayName: event.target.value } : current))} placeholder="Display name (optional)" className="rounded-lg border border-slate-200 px-2 py-2 text-xs"/><button onClick={() => setMappingDrafts((rows) => rows.filter((_, i) => i !== index))} title="Remove mapping" className="grid place-items-center rounded-lg border border-slate-200 text-slate-400 hover:text-rose-600"><XCircle size={14}/></button></div>)}</div></div>}
