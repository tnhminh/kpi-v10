...
function message(error: unknown) {
  return error instanceof ClientApiError
    ? `${error.message}${error.requestId ? ` Â· Request ${error.requestId}` : ""}`
...
function CriterionEditor({
  ...
  return <div className="space-y-4">
    <Panel className="p-5">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">{(["AUTO", "ASSISTED", "MANUAL"] as const).map((value) => <button disabled={!mutable} key={value} onClick={() => setMethod(value)} className={`rounded-lg border p-3 text-left disabled:cursor-not-allowed ${method === value ? "border-blue-400 bg-blue-50" : "border-slate-200"}`}><div className="text-xs font-bold">{value}</div><div className="mt-1 text-[10px] leading-4 text-slate-500">{value === "AUTO" ? "Metric â†’ deterministic rule":value === "ASSISTED" ? "Suggested score â†’ human review":"Evidence â†’ human score"}</div></button>)}</div>
    ...
    {method !== "MANUAL" && <Panel className="p-5">
      ...
      <div className="mt-4 space-y-2">{bands.map((band, index) => <div key={index} className="grid grid-cols-[90px_1fr_30px_1fr_34px] items-center gap-2">
...
export default function PersistedKpiBuilder({ organization, selectedTemplateId, onTemplateChange }: BuilderProps) {
  ...
  if (templatesQuery.isPending) return <Panel className="p-8"><div className="flex items-center justify-center gap-2 text-sm text-slate-500"><LoaderCircle size={16} className="animate-spin"/> Loading KPI templatesâ€¦</div></Panel>;
  if (templatesQuery.error) return <Panel className="border-rose-200 p-6"><InlineError error={templatesQuery.error}/></Panel>;
  if (!templates.length) return <Panel className="p-8 text-center"><div className="text-sm font-semibold">No KPI template available</div><p className="mt-1 text-xs text-slate-500">Create a persistent KPI template first, then open its builder.</p></Panel>;

  return <div className="-m-7 flex h-[calc(100vh-65px)] flex-col overflow-hidden">
    <div className="border-b border-slate-200 bg-white px-5 py-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <select value={activeTemplateId ?? ""} onChange={(event) => { setSelectedVersionId(null); setSelectedCriterionId(null); onTemplateChange(event.target.value); }} className="max-w-64 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold">{templates.map((template: KpiTemplateDto) => <option key={template.id} value={template.id}>{template.name}</option>)}</select>
          {versionsQuery.isPending ? <span className="text-xs text-slate-400">Loading versionsâ€¦</span> : versions.length > 0 && <select value={activeVersionId ?? ""} onChange={(event) => { setSelectedVersionId(event.target.value); setSelectedCriterionId(null); }} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold">{versions.map((version) => <option key={version.id} value={version.id}>v{version.version} Â· {version.status}{version.submittedAt && version.status === "DRAFT" ? " Â· submitted" : ""}</option>)}</select>}
          {detail && <StatusBadge>{detail.status}{detail.submittedAt && detail.status === "DRAFT" ? " Â· frozen" : ""}</StatusBadge>}
        </div>
